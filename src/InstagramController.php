 ...
 * @since 2015-05-18
 **/
 include 'InstagramDBHandler.php';
...
class InstagramController
{
     ...
     */
	 private $_instagramClient;
     private $_dbHandler;
	 
	//Default constructor
    public function __construct($config){ $this->_instagramClient = new Instagram($config);	}
	

	//Public Metods
	//Get Media	
	//Returns JSON object filled with the Media and Geo positioning information.
    public function getMedia($id)
    {
	  //Set the default status code, error type and message
	  $respValues = array("code"=>200,
	                      "error_type" => "",
						  "error_message"=>"",
						  "data"=>"");
      ...
      $arr = array('id'=> 0, 
		           'location'=> array ('geopoint'=> array('latitude'=>0, 
		                                                  'longitude' =>0),
									   'extra_data'=> array('country'=>'',
									                        'state'=>'',
									                        'place'=>'',
									                        'fclnames'=>''
															)
	  ...
	  if ($obj->getId() > 1){
		  $arr['id'] = $obj->getId();
		  $arr['location'] = array ('geopoint'=> array('latitude'=>$obj->getLat(), 
					                                          'longitude' =>$obj->getLong()),
										   'extra_data'=> array('country'=>$obj->getCountry(),
										                        'state'=>$obj->getState(),
										                        'place'=>$obj->getPlace(),
		...
		try 
		{
		  ...
		  if (($respValues['code'] == 200) && ($response->getHeader('Content-Length') > 0)) {
			$arr['id'] = $id;
		    ...
		    if ($respValues['code'] == 200){
			  ...
			  $arr['location'] = array ('geopoint'=> array('latitude'=>$lat, 
					                                          'longitude' =>$lng),
										   'extra_data'=> array('country'=>$geoData->geonames[0]->countryName,
										                        'state'=>$geoData->geonames[0]->adminName1,
										                        'place'=>$geoData->geonames[0]->name,
										                        'fclnames'=>$geoData->geonames[0]->fclName
																));
				 if (!$this->_dbHandler->insertMedia(array("id" => $id,
				                                      "lat"=> $lat,
													  "lng" => $lng,
